import 'package:reborn_packaging/features/home/models/collection_item.dart';

const mockCollections = <CollectionItem>[
  CollectionItem(
    title: 'KRAFT ROUND BOWLS',
    handle: 'kraft-round-bowls',
    imageAsset: 'assets/images/collections/kraft_round_bowls.png',
  ),
  CollectionItem(
    title: 'KRAFT RECTANGULAR BOWLS',
    handle: 'kraft-rectangular-bowls',
    imageAsset: 'assets/images/collections/kraft_rectangular_bowls.png',
  ),
  CollectionItem(
    title: 'NAPKINS',
    handle: 'napkins',
    imageAsset: 'assets/images/collections/napkins.png',
  ),
  CollectionItem(
    title: 'PAPER CARRIER BAGS',
    handle: 'paper-carrier-bags',
    imageAsset: 'assets/images/collections/paper_carrier_bags.png',
  ),
  CollectionItem(
    title: 'KRAFT BOAT TRAYS',
    handle: 'kraft-boat-trays',
    imageAsset: 'assets/images/collections/kraft_boat_trays.png',
  ),
  CollectionItem(
    title: 'PAPER COFFEE CUPS',
    handle: 'paper-coffee-cups',
    imageAsset: 'assets/images/collections/paper_coffee_cups.png',
  ),
  CollectionItem(
    title: 'KRAFT BOAT TRAYS',
    handle: 'kraft-boat-trays',
    imageAsset: 'assets/images/collections/kraft_boat_trays.png',
  ),
  CollectionItem(
    title: 'PAPER COFFEE CUPS',
    handle: 'paper-coffee-cups',
    imageAsset: 'assets/images/collections/paper_coffee_cups.png',
  ),
];
